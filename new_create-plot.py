...
def main():
    ...
    for key, df in dataframes.items():
        ...
        plt.plot(resampled_data.index.to_numpy(), resampled_data['a5_3'].to_numpy(), marker='o', linestyle='-',
                 color='red')
        plt.plot(resampled_data.index.to_numpy(), resampled_data['a5_4'].to_numpy(), marker='o', linestyle='-',
                 color='green')
        plt.title(
            f'Algorithm Occurrences for {provider} BTS in Euskirchen, LAC: {key.split("_")[2]} / Total Samples: {len(df)}')
        plt.xlabel('Time')
